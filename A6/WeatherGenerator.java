 ...
 */
 ...
 public class WeatherGenerator {
        ...
        for(int x = 0; x < drywet.length; x++){
            if(drywet[x][0] == longitude && drywet[x][1] == latitude){
                for(int y = 0; y < drywetProbability.length; y++){
                    drywetProbability[y] = drywet[x][y + 2];
                }
                for(int z = 0; z < wetwetProbability.length; z++){
                    wetwetProbability[z] = wetwet[x][z + 2];
                }
            }
        }
    }
    ...
    public static int[] forecastGenerator(double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        // COMPLETE THIS METHOD
        int[] newresult = new int[numberOfDays];
        int con = 0;
        double num = 0;
        for(int x = 0; x < numberOfDays; x++){
           num = StdRandom.uniform();
            if(x == 0){
                if (num <= 0.5){
                 con = 1;
                    newresult[x] = 1;
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        // COMPLETE THIS METHOD
        double [][] DW = new double [numberOfLocations][14];
        double [][] WW = new double [numberOfLocations][14];
        populateArrays(DW, WW);

        double [] dryProb = new double [12];
        double [] wetProb = new double [12];
        populateLocationProbabilities(dryProb, wetProb, longitude, latitude, DW, WW);

        int days = numberOfDaysInMonth[month];
        double wet = wetProb[month];
        double dry = dryProb[month];
        return forecastGenerator(dry, wet, days);
    }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int i = 0; i < forecast.length; i++){
            if(forecast[i] == mode){
                ...
                int temp2 = temp;
                temp = 0;
                if (temp > longestDays) longestDays = temp2;
            }
        }
        return longestDays;
    }
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        int count = 0;
        int maxDry = 0;
        for(int week = 0; week < forecast.length-6; week++){
            count = 0;
            for(int day = week; day < week+7; day++){
                if(forecast[day] == 2) count++;
            }
            if(count > maxDry){
                maxDry = count;
                index = week;
