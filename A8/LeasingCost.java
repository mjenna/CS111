public class LeasingCost {
    ...
    public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        for (Vehicle v : vehicles) {
            double length = v.getLease().getLeaseLength();
            double usage = v.getFuel().getUsage();
            double allowance = v.getLease().getMileageAllowance();
            double due = v.getLease().getDueAtSigning();
            double cost = v.getLease().getMonthlyCost();
            double charger = v.getFuel().getCharger();
            double co2PerUnit;
            double price;

            if (v.getFuel().getType() == 1) {
                co2PerUnit = v.getFuel().CO2EMITTED_GASCOMBUSTION;
                price = gasPrice;
            } else {
                co2PerUnit = v.getFuel().CO2EMITTED_ELECTRICITYCOMBUSTION;
                price = electricityPrice;
            }

            double emmision = computeCO2(length, usage, allowance, co2PerUnit);
            double fuelCost = computeFuelCost(length, usage, allowance, price);
            double leaseCost = computeLeaseCost(due, (int) length, cost);
            v.setCO2Emission(emmision);
            v.setFuelCost(fuelCost);

            double total;

            if (v.getFuel().getType() == 1) {
                total = fuelCost + leaseCost;
            } else {
                total = fuelCost + leaseCost + charger;
            }
